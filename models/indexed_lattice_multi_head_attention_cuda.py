...
class DS_MultiHeadAttention(Module):
    """
    DSformer dedicated Attention module.
    """
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, 
                 kdim=None, vdim=None, 
                 params=LatticeformerParams(),
                 device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        super(DS_MultiHeadAttention, self).__init__()
        ...
        # --- Angular feature fusion layer---
        # The input dimension here is d_model (from Angle Encoder), and the output dimension is num_heads.
        # We need to generate a Bias for each Head.
        self.angle_bias_proj = nn.Linear(embed_dim, num_heads, bias=False, **factory_kwargs)
    ...
    def _reset_parameters(self):
        ...
        # Initialize angle_bias_proj
        xavier_uniform_(self.angle_bias_proj.weight)
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor, batch_q:Tensor, batch_kv:Tensor, edges: Tensor,
                    ...
                    need_weights: bool = True,
                    #####
                    angle_emb: Optional[Tensor] = None,       
                    local_edge_mask: Optional[Tensor] = None  # Note: The actual value passed here is local_edge_index.
                    ) -> Tuple[Tensor, Optional[Tensor]]:

    
        pe_dist_proj = self.pe_dist_proj
        pe_wave_proj = self.pe_wave_proj

       
        tgt_len, embed_dim = query.shape
        # src_len, _ = key.shape # not used
        # esz = edges.shape[1]   # not used
        
        head_dim_q = self.q_proj_weight.shape[0] // self.num_heads
        head_dim_k = self.k_proj_weight.shape[0] // self.num_heads
        head_dim_v = self.v_proj_weight.shape[0] // self.num_heads

        #  In-Projection
        if self.in_proj_bias is None:
            b_q = b_k = b_v = None
        else:
            b_q, b_k, b_v = self.in_proj_bias.split([self.q_proj_weight.size(0), self.k_proj_weight.size(0), self.v_proj_weight.size(0)])
        
        q, k, v = _in_projection(query, key, value, self.q_proj_weight, self.k_proj_weight, self.v_proj_weight, b_q, b_k, b_v)
        
        q = q.contiguous().view(tgt_len, self.num_heads, head_dim_q)
        k = k.contiguous().view(k.shape[0], self.num_heads, head_dim_k)
        v = v.contiguous().view(v.shape[0], self.num_heads, head_dim_v)
        ...
        if self.params.gauss_state.startswith("q"):
             alpha = q.reshape(tgt_len, self.num_heads, 1, head_dim_q) @ W.reshape(self.num_heads, head_dim_q, 1)
        elif self.params.gauss_state == "1":
                ones = torch.ones_like(q)
                alpha = ones.reshape(tgt_len, self.num_heads,1, head_dim_q) @ W.reshape(self.num_heads, head_dim_q, 1)
        else:
             alpha = F.linear(query, W)
        alpha = alpha.view(tgt_len, self.num_heads)
        
        if self.params.normalize_gauss:
            scale = self.gauss_scale[0]
            shift = self.gauss_scale[1]
            if scale == 0:
                 scale = torch.reciprocal(alpha.detach().std())
                 shift = -alpha.detach().mean()
                 if self.training:
                    self.gauss_scale[0] = scale
                    self.gauss_scale[1] = shift
            alpha = (alpha + shift) * scale
        
        output_global, attn_weights_global = crystalformer_multi_head_attention_forward_cuda(
            query, key, value, batch_q, batch_kv, edges, 
            points, dist2_min, tvecs,
            kr_base, rvecs, vcell, 
            W,
        ...
        # 'local_edge_mask' here is actually 'local_edge_index' [2, E_local].
        local_edge_index = local_edge_mask 
        
        if angle_emb is not None and local_edge_index is not None:
            # 1. Q, K, V (Flat View)
            # q: [N_nodes, Heads, Head_Dim]
            q_flat = q.squeeze(1) if q.dim() == 4 else q
            k_flat = k.squeeze(1) if k.dim() == 4 else k
            v_flat = v.squeeze(1) if v.dim() == 4 else v
            
            # 2. Gather based on Local Edges
            src_idx = local_edge_index[0]
            dst_idx = local_edge_index[1]
            
            q_local = q_flat[src_idx] # [E_local, H, D]
            k_local = k_flat[dst_idx] # [E_local, H, D]
            v_local = v_flat[dst_idx] # [E_local, H, D]
            
            # 3. Calculate Scores
            # Score = (Q * K) / sqrt(d)
            score_local = (q_local * k_local).sum(dim=-1) / math.sqrt(self.head_dim) # [E_local, H]
            
            # 4. Inject Angle Bias
            # angle_emb: [E_local, D_model] -> [E_local, H]
            angle_bias = self.angle_bias_proj(angle_emb)
            score_local = score_local + angle_bias
            
            # 5. Softmax & Aggregate
            # Softmax over neighbors (dst) for each center (src)
            alpha_local = pyg_softmax(score_local, src_idx, dim=0, num_nodes=tgt_len) # [E_local, H]
            
            # Weighted Sum: sum(alpha * V)
            # Use scatter for aggregation
            out_local_raw = scatter(alpha_local.unsqueeze(-1) * v_local, src_idx, dim=0, reduce='sum', dim_size=tgt_len) # [N, H, D]
            
            # 6. Output Projection
            # Flatten: [N, H, D] -> [N, H*D]
            out_local_raw = out_local_raw.view(tgt_len, self.num_heads * self.head_dim)
            output_local = self.out_proj(out_local_raw)

        # ==========================================================
        # Merge Streams
        # ==========================================================
        
        # Final Output = Global + Local
        output_final = output_global + output_local
        
        if need_weights:
            # Returns the global weights as a reference (local weights are usually not needed)
            # Note: the attn_weights returned by output_global are already averaged [E_global]
            # We pass it through directly
            return output_final, attn_weights_global
        else:
