...
class Periodic_Atomic_Encoding(nn.Module):
    "PAE"
    def __init__(self, emb_input_dim, emb_output_dim, per_ratio=0.15, num_freqs=16):
        ...
        acti_output_dim = emb_output_dim - per_output_dim * 2 

        self.input_linear = nn.Linear(emb_input_dim, emb_input_dim)
        self.per_linear = nn.Linear(emb_input_dim, per_output_dim)
        self.acti_linear = nn.Linear(emb_input_dim, acti_output_dim)
    ...
    def forward(self, x):
        ...
        cos_feat, sin_feat = torch.cos(self.per_linear(x)), torch.sin(self.per_linear(x))
        acti_feat = self.activation(self.acti_linear(x))
                
        outputs = torch.cat((cos_feat, sin_feat, acti_feat), dim=-1)
...
class Latticeformer(torch.nn.Module):
    ...
    def __init__(self, params):
        ...
        # self.input_embeddings = Pre_Embedding(self.ATOM_FEAT_DIM, model_dim)
